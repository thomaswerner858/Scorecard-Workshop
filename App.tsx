...
const App: React.FC = () => {
  ...
  const triggerAiFeedback = async () => {
    ...
    try {
      ...
      setError(`Fehler im Sparring: ${err.message || "Unbekannter Fehler"}`);
    } finally {
      setLoadingAi(false);
    }
  };
  ...
  const handleAiGeneration = async () => {
    ...
    try {
      ...
      if (config.groups && config.parameters) {
        setGroups(config.groups);
        setParameters(config.parameters);
        setAiFeedback("✨ KI-Modell wurde erfolgreich erstellt.");
      }
      setGenerationPrompt('');
    } catch (err: any) {
      setError(`Generator-Fehler: ${err.message || "Prüfen Sie Ihren API_KEY"}`);
    } finally {
  ...
  const exportConfiguration = () => {
    ...
    a.href = url;
    a.download = `scoring-studio-config-${Date.now()}.json`;
    a.click();
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-indigo-200">B</div>
          ...
          <div className="flex items-center gap-4">
             ...
             <button onClick={exportConfiguration} className="bg-slate-900 text-white px-6 py-2.5 rounded-xl text-xs font-black hover:bg-slate-800 transition flex items-center gap-2 active:scale-95 shadow-lg">
        ...
        <div className="mb-14 bg-white p-2 rounded-[2.5rem] border border-slate-200 shadow-2xl shadow-indigo-100/40">
          <div className="bg-slate-50 p-6 rounded-[2rem] flex flex-col md:flex-row gap-5 items-center">
            ...
            <div className="flex-1 w-full">
              <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">Automatischer Modell-Generator</p>
              <input 
                type="text" 
                value={generationPrompt}
                onChange={(e) => setGenerationPrompt(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleAiGeneration()}
                placeholder="Beschreiben Sie Ihre Zielgruppe oder Branche..."
            ...
            <button 
              ...
              className="w-full md:w-auto bg-indigo-600 text-white px-10 py-4 rounded-xl font-black text-xs uppercase hover:bg-indigo-700 disabled:opacity-50 transition shadow-xl active:scale-95 flex items-center justify-center gap-3"
            >
              {generatingConfig ? <Loader2Icon className="animate-spin" size={18} /> : <SparklesIcon size={18} />} 
              {generatingConfig ? 'Generiere...' : 'Erstellen'}
            </button>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-16">
              ...
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-black text-white text-xl tracking-tight uppercase">Strategisches Sparring</h3>
                      <p className="text-indigo-400 text-[10px] font-black uppercase tracking-widest">KI Risk Manager</p>
                    </div>
                  </div>
                  <button 
                    onClick={triggerAiFeedback} 
                    disabled={loadingAi} 
                    className="bg-white text-slate-900 px-8 py-3 rounded-xl text-xs font-black uppercase hover:bg-indigo-50 transition active:scale-95 disabled:opacity-50"
                  >
                ...
                {aiFeedback ? (
                  ...
                  <div className="text-center py-10">
                    <p className="text-slate-500 text-sm italic max-w-md mx-auto">
                      Klicken Sie auf "Analyse Starten", um Ihr aktuelles Modell von unserem KI-Experten validieren zu lassen.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm">
              <h2 className="text-xl font-black text-slate-900 uppercase mb-10 flex items-center gap-3">
                <FileTextIcon size={20} className="text-slate-400" /> Modell Übersicht
              </h2>
                  ...
                  <div key={group.id} className="group-item">
                    <div className="flex justify-between items-end mb-4 border-b border-slate-100 pb-3">
                      ...
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] text-slate-400 font-bold uppercase">Gruppengewicht</span>
                      ...
                      {parameters.filter(p => p.groupId === group.id).map(p => (
                        <div key={p.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 hover:border-indigo-200 transition-colors">
                          <div className="flex justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black text-slate-400 bg-white px-2 py-1 rounded border border-slate-100">{p.weight}%</span>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {p.ranges.map(r => (
                              <span key={r.id} className="text-[9px] bg-white border px-2 py-1 rounded-md text-slate-500 font-bold shadow-sm">
